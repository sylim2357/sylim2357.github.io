...
만약 군 $$G$$가 벡터 공간 $$V$$에 작용한다면 그 작용을 표현 (representation)이라고 하고, $$\pi:G\rightarrow GL(V)$$로 나타낸다. $$GL(V)$$는 벡터공간 $$V$$의 일반선형군 (General Linear Group) 이며, 가역 선형변환이 이루는 군이다. (Invertible Linear Maps on $$V$$) 벡터가 이산적이라면 벡터의 원소는 이산적인 인덱스를 가지고 있고, 만약 벡터의 인덱스에서 정의된 군 작용이 있다고 하면 이를 벡터의 인덱스로 확장시킬 수 있다. 자연스럽게 이 군 표현은 $$(\pi(g)v)[i]\coloneqq v[g^{-1}i]$$로 나타낼 수 있다. 벡터의 원소는 정수값을 가지고 있기 때문에 위에서 정의한 것을 그대로 가져오게 된다면 $$(\pi(g)v)[i]=v[g^{-1}i]=v[i-g] \enspace\forall g,i\in\mathbb{Z}$$가 된다.
